...
def image_process(img):
    ...
    for coords in pose_preds:
        ...
        if left_shoulder.any() and right_shoulder.any(): #jd1
            cv2.line(image_debug, (int(left_shoulder[0]), int(left_shoulder[1])), (int(right_shoulder[0]), int(right_shoulder[1])), (100, 171, 231), 2)
            jds_list.append( dist(left_shoulder, right_shoulder) )
        if left_shoulder.any() and left_hip.any(): #jd2
            cv2.line(image_debug, (int(left_shoulder[0]), int(left_shoulder[1])), (int(left_hip[0]), int(left_hip[1])), (100, 171, 231), 2)
            jds_list.append( dist(left_shoulder, left_hip) )
        if left_shoulder.any() and left_elbow.any(): #jd3
            cv2.line(image_debug, (int(left_shoulder[0]), int(left_shoulder[1])), (int(left_elbow[0]), int(left_elbow[1])), (100, 171, 231), 2)
            jds_list.append( dist(left_shoulder, left_elbow) )
        if left_elbow.any() and left_wrist.any(): #jd4
            cv2.line(image_debug, (int(left_elbow[0]), int(left_elbow[1])), (int(left_wrist[0]), int(left_wrist[1])), (100, 171, 231), 2)
            jds_list.append( dist(left_elbow, left_wrist) )
        if right_hip.any() and right_shoulder.any(): #jd5
            cv2.line(image_debug, (int(right_shoulder[0]), int(right_shoulder[1])), (int(right_hip[0]), int(right_hip[1])), (100, 171, 231), 2)
            jds_list.append( dist(right_hip, right_shoulder) )
        if right_elbow.any() and right_shoulder.any(): #jd6
            cv2.line(image_debug, (int(right_shoulder[0]), int(right_shoulder[1])), (int(right_elbow[0]), int(right_elbow[1])), (100, 171, 231), 2)
            jds_list.append( dist(right_elbow, right_shoulder) )
        if right_wrist.any() and right_elbow.any(): #jd7
            cv2.line(image_debug, (int(right_elbow[0]), int(right_elbow[1])), (int(right_wrist[0]), int(right_wrist[1])), (100, 171, 231), 2)
            jds_list.append( dist(right_wrist, right_elbow) )
        if right_hip.any() and left_hip.any():   #jd8
            cv2.line(image_debug, (int(left_hip[0]), int(left_hip[1])), (int(right_hip[0]), int(right_hip[1])), (100, 171, 231), 2)
            jds_list.append( dist(left_hip, right_hip) )
        if left_hip.any() and left_knee.any(): #jd9
            cv2.line(image_debug, (int(left_hip[0]), int(left_hip[1])), (int(left_knee[0]), int(left_knee[1])), (100, 171, 231), 2)
            jds_list.append( dist(left_hip, left_knee) )
        if right_hip.any() and right_knee.any():  #jd10
            cv2.line(image_debug, (int(right_hip[0]), int(right_hip[1])), (int(right_knee[0]), int(right_knee[1])), (100, 171, 231), 2)
